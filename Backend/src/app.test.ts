...
describe("CORS", () => {
  it("allows requests from the configured frontend", async () => {
    const response = await request(app).get("/cors-test").set("Origin", frontendUrl);
  ...
  it("does not authorize requests from another origin", async () => {
    const response = await request(app).get("/cors-test").set("Origin", "https://untrusted.example");
