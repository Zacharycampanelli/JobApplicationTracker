  ...
  return (
    <div className="mx-auto flex min-h-dvh w-full max-w-5xl flex-col bg-surface px-6 py-4 md:relative">
      ...
      <div className="mt-8 grid gap-6">
        ...
        <Button icon={<Save />} type="button" onClick={()} >
          Save Changes
        </Button>

        <Input id="name" label="FULL NAME " {...register("name")} defaultValue={user?.name} error={errors.name?.message} />

        <Input id="email" label="EMAIL ADDRESS" disabled defaultValue={user?.email} />
        </form>
      </div>
    </div>
  )
